...
def test_drive_result(car, time_series):
    start_step = 5
    time_stamp = step_to_timestamp(time_series, start_step)
    car.drive(time_stamp, start_step, 10, Location(), 0.45, 1, 0)
    assert car.soc == 0.45
...
def test_drive_bad_soc(car, time_series):
    ...
    with pytest.raises(
        ...
        car.drive(time_stamp, start_step, 60, Location(), 0.7, 1, 0)
...
def test_drive_input_checks(car, time_series):
    ...
    with pytest.raises(TypeError, match="Argument has wrong type."):
        car.drive(
            time_stamp, start_step, "60", Location(), 0.7, 1, 0
        )
...
def test_charge_result(car, time_series):
    ...
    time_stamp = step_to_timestamp(time_series, start_step)
    car.charge(
        time_stamp, start_step, 60, 11, 0.8, 11, 0
    )
    assert car.soc == 0.8
...
def test_charge_bad_soc(car, time_series):
    ...
    with pytest.raises(
        ...
        car.charge(
            time_stamp, start_step, load_level=0, time=60, power=11, new_soc=0.3, charging_capacity=11
        )
...
def test_charge_sanity(car, time_series):
    ...
    with pytest.raises(
        ...
        car.charge(
            time_stamp, start_step, load_level=0, time=2, power=7, new_soc=0.9, charging_capacity=11
        )
